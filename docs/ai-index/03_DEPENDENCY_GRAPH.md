...
19. js/features/session-search.js (S105 → S003, S015, S016, S017, S113)
19b.js/features/session-filter.js (S113 → S105, S016)
20. js/features/session-history.js (S015 → S001, S002, S003, S004, S005, S016, S017, S020, S105)
...
S015 —[USES]→ S105  (SessionHistoryManager → SessionSearchManager.exitResultsView/isSearchActive/_performSearch)
...
S105 —[USES]→ S017  (SessionSearchManager → TranscriptManager.getSpeakerColorMap/highlightText)
S105 —[USES]→ S015  (SessionSearchManager → SessionHistoryManager.loadSessionFromHistory)
S105 —[USES]→ S003  (SessionSearchManager → UIManager.updateUIState for view restore)
S113 —[USES]→ S105  (SessionFilterManager → SessionSearchManager._performSearch for re-render)
S113 —[USES]→ S016  (SessionFilterManager → SessionUIManager.renderSessionHistory fallback)
S016 —[USES]→ S113  (SessionUIManager → SessionFilterManager.hasActiveFilters for empty state)
...
S122 —[USES]→ S006  (ModalManager.showPromptOnboardingModal → AppConstants.STORAGE_KEYS.LAST_SEEN_VERSION, PROMPT_ONBOARDING_VERSION)
S122 —[USES]→ S100  (ModalManager.showPromptOnboardingModal → SettingsManager.setDefaultPrompt)
S067 —[USES]→ S122  (initializeApplication → ModalManager.showPromptOnboardingModal after state restoration)
